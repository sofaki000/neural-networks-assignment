...
def save_model_train_and_test_metric(metric, train_metric , test_metric, title, file_name ):
    ...
    if metric=='loss':
        ...
        # ax.plot(epochs_index,train_metric, color='r', label='Train loss')
        # ax.plot(epochs_index,test_metric, color='g', label='Test loss')
...
def save_model_train_and_test_accuracy_plot(train_acc, test_acc, title, file_name):
    save_model_train_and_test_metric('acc',train_acc , test_acc, title, file_name)

def save_multiple_plots_for_two_experiments(experiments_mean_losses, titles, file_name):
    file_name = strftime(f"{file_name}Date%Y_%m_%d_Time%H_%M_%S", gmtime())
    figure, axis = plt.subplots(2, sharex=True ,figsize=(7,7), gridspec_kw={'width_ratios': [3], 'height_ratios': [3,2], 'wspace': 0.4,  'hspace': 0.4})
    k = len(experiments_mean_losses) - 1
    # figure.tight_layout()
